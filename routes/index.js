...
router.post("/vote", [check("email").isEmail(), check("name").isLength({ min: 1, max: 10 }), check("code").isLength(4), check("phone").notEmpty(), check("votes").isArray({ min: 1, max: 5 })], indexController.postVote);
